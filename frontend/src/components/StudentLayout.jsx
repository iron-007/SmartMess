...
const StudentLayout = () => {
  ...
  return (
    <div className="bg-light min-vh-100">
      ...
      {/* --- THE SIDEBAR --- */}
      <aside className={`admin-sidebar shadow-lg ${isSidebarOpen ? 'open' : ''}`}>
        {/* Brand/Logo */}
      ...
      <div className="admin-main-wrapper">
        ...
        <header className="admin-top-nav d-flex justify-content-between align-items-center px-3 px-md-4">
          <div className="d-flex align-items-center gap-2 gap-md-3">
            ...
            >
              <i className="bi bi-hexagon-fill fs-4 text-gradient"></i>
            </button>
            <div className="d-flex flex-column">
              <h4 className="m-0 fs-6 fs-md-5 fw-bold nav-title text-truncate" style={{ maxWidth: '140px' }}>Student Portal</h4>
              <small className="text-muted fw-medium mt-1 d-none d-md-block">Welcome back, {user.name}</small>
            </div>
          </div>
          ...
          <div className="d-flex align-items-center gap-2 gap-md-4">
            <div className="d-flex align-items-center gap-2">
              <div className="bg-light rounded-circle d-flex align-items-center justify-content-center border" style={{ width: '36px', height: '36px' }}>
                <i className="bi bi-mortarboard-fill text-secondary fs-6"></i>
              </div>
              <div className="d-flex flex-column">
                <span className="fw-bold text-dark lh-1" style={{ fontSize: '0.85rem' }}>{user.name}</span>
                <span className="text-muted d-none d-sm-block" style={{ fontSize: '0.75rem' }}>Student</span>
              </div>
            </div>
